...
describe('The VBScript transpiler - Redim', () => {
	it('should transpile a one-dimension redim', () => {
		...
		expect(js).to.equal(`let myarray = ${ReferenceTransformer.VBSHELPER_NAME}.redim(myarray, [2]);`);
	});

	it('should transpile a one-dimension redim with preserve', () => {
		const vbs = `Redim Preserve myarray(2)\n`;
		const js = vbsToJs(vbs);
		expect(js).to.equal(`let myarray = ${ReferenceTransformer.VBSHELPER_NAME}.redim(myarray, [2], true);`);
	});

	it('should transpile a multi-dimension redim', () => {
		const vbs = `Redim myarray(2,4,3)\n`;
		const js = vbsToJs(vbs);
		expect(js).to.equal(`let myarray = ${ReferenceTransformer.VBSHELPER_NAME}.redim(myarray, [\n    2,\n    4,\n    3\n]);`);
	});
	...
	it('should transpile a redim with multiple arrays', () => {
		...
		const js = vbsToJs(vbs);
		expect(js).to.equal(
			`let myarray = ${ReferenceTransformer.VBSHELPER_NAME}.redim(myarray, [\n        2,\n        4,\n        3\n    ]), myarray2 = ${ReferenceTransformer.VBSHELPER_NAME}.redim(myarray2, [100]);`,
		);
	});
	...
	it('should transpile a redim with multiple arrays and preserve', () => {
		...
		const js = vbsToJs(vbs);
		expect(js).to.equal(
			`let myarray = ${ReferenceTransformer.VBSHELPER_NAME}.redim(myarray, [\n        2,\n        4,\n        3\n    ], true), myarray2 = ${ReferenceTransformer.VBSHELPER_NAME}.redim(myarray2, [100], true);`,
		);
	});
});
